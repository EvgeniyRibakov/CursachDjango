...
class SendNewsletterView(LoginRequiredMixin, View):
    def post(self, request, newsletter_id):
        try:
            newsletter = Newsletter.objects.get(id=newsletter_id, owner=request.user)
            send_newsletter.delay(newsletter.id)
            return JsonResponse({"status": "success", "message": "Newsletter sending started"})
        except Newsletter.DoesNotExist:
            return JsonResponse({"status": "error", "message": "Newsletter not found or access denied"}, status=404)
